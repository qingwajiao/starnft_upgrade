...
interface IstarNFT is IERC721Upgradeable {
...
contract Staking is Initializable, IERC721ReceiverUpgradeable, OwnableUpgradeable {
    ...
    function stake(address to, uint256 tokenId)external {
        uint256 power =  IstarNFT(_StartNft).getNFTpower(tokenId);
        ownerOf[tokenId] = to;
    ...
    function withdraw(uint256 tokenId)external {
        ...
        updatePool();
        // 转nft给用户
        IstarNFT(_StartNft).safeTransferFrom(address(this),msg.sender,tokenId);
    }
